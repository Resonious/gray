// Code by Campbell Barton (ideasman42@gmail.com)
// https://gitlab.com/ideasman42/rust-simple-examples/blob/master/polyfill_2d
// Which in turn came from LibGDX
...
pub const CONCAVE: i8 = -1;
pub const TANGENTIAL: i8 = 0;
pub const CONVEX: i8 = 1;
...
fn area_tri_signed_v2_alt_2x(v1: &[Real; 2], v2: &[Real; 2], v3: &[Real; 2]) -> Real {
    ((v1[0] * (v2[1] - v3[1])) + (v2[0] * (v3[1] - v1[1])) + (v3[0] * (v1[1] - v2[1])))
}

fn span_tri_v2_sign(v1: &[Real; 2], v2: &[Real; 2], v3: &[Real; 2]) -> ESign {
...
///
fn cross_poly_v2(coords: &Vec<[Real; 2]>) -> Real {
...
/*
fn area_poly_v2(coords: &Vec<[Real; 2]>) -> Real {
    (0.5 * cross_poly_v2(coords)).abs()
}

fn cross_tri_v2(v1: &[Real; 2], v2: &[Real; 2], v3: &[Real; 2]) -> Real {
    (v1[0] - v2[0]) * (v2[1] - v3[1]) +
    (v1[1] - v2[1]) * (v3[0] - v2[0])
}

fn area_tri_signed_v2(v1: &[Real; 2], v2: &[Real; 2], v3: &[Real; 2]) -> Real {
    0.5 * ((v1[0] - v2[0]) * (v2[1] - v3[1]) + (v1[1] - v2[1]) * (v3[0] - v2[0]))
}

fn area_tri_v2(v1: &[Real; 2], v2: &[Real; 2], v3: &[Real; 2]) -> Real {
...
pub const KDNODE_UNSET: usize = ::std::usize::MAX as usize;
...
pub struct KDTree2D<'a> {
    ...
    root: usize,
    pub totnode: usize,
    nodes_map: Vec<usize>, /* index -> node lookup */
}
...
fn new_kdtree<'a>(
    ...
    nodes_map: Vec<usize>,
    tot: usize,
    coords: &'a Vec<[Real; 2]>) -> KDTree2D<'a>
{
    KDTree2D {
        nodes: nodes,
        coords: coords,
...
fn init(tree: &mut KDTree2D, indices: &Vec<PolyIndex>) {
    for pi in indices {
        if pi.sign != CONVEX {
            tree.nodes.push(
                KDTreeNode2D {
                    parent: KDNODE_UNSET, // could avoid setting here!
                    neg: KDNODE_UNSET,
                    pos: KDNODE_UNSET,
                    index: pi.index,
                    axis: 0,
                ...
                loop {
                    unsafe {
                        while coords.get_unchecked(
                            nodes.get_unchecked(
                                {i = i.wrapping_add(1); i}).index)[axis] < co {}
                        while coords.get_unchecked(
                            nodes.get_unchecked(
                                {j = j.wrapping_sub(1); j}).index)[axis] > co && j > neg {}
                    }
...
fn init_mapping(tree: &mut KDTree2D) {
...
fn node_remove(tree: &mut KDTree2D, index: usize) {
        ...
        ) -> bool
{
    ...
    let bounds: [KDRange2D; 2] = [
        KDRange2D { min: vs[0][0].min(vs[1][0]).min(vs[2][0]),
                    max: vs[0][0].max(vs[1][0]).max(vs[2][0]), },
        KDRange2D { min: vs[0][1].min(vs[1][1]).min(vs[2][1]),
    ...
    let prev;
    {
        ...
        if USE_KDTREE {
            ...
            if pf.kdtree.totnode != 0 {
                node_remove(&mut pf.kdtree, pi.index);
            }
        }
    }
...
fn ear_tip_cut(pf: &mut PolyFill, pi_ear_tip_index: usize) {
    {
        ...
        pf.tris.push([pf.indices[pi_ear_tip.prev].index as usize,
                        pi_ear_tip.index as usize,
                        pf.indices[pi_ear_tip.next].index as usize]);
    }
...
fn triangulate(pf: &mut PolyFill) {
    ...
    if pf.coords_tot == 3 {
        ...
        pf.tris.push([
            pf.indices[pi_ear_tip.prev].index as usize,
            pi_ear_tip.index as usize,
            pf.indices[pi_ear_tip.next].index as usize,
        ]);
    }
...
fn prepare<'a>(
    ...
    r_tris: &'a mut Vec<[usize; 3]>,
    mut indices: Vec<PolyIndex>,
) -> PolyFill<'a>
{
    if coords_sign == 0 {
        coords_sign = if cross_poly_v2(coords) >= 0.0 { 1 } else { -1 };
    }
    ...
    let kdtree =
        if coords_tot_concave == 0 {
            new_kdtree(vec![], vec![], 0, coords)
        } else {
            let mut kdtree = new_kdtree(
                Vec::with_capacity(coords_tot_concave),
                vec![KDNODE_UNSET; coords.len()],
                coords_tot_concave,
                coords);

            init(&mut kdtree, &indices);
            balance(&mut kdtree);
            init_mapping(&mut kdtree);
            kdtree
        };
...
fn calc(pf: &mut PolyFill) {
    triangulate(pf);
}
// END POLYFILL

///
/// This function takes 2D `coords` and fills in `r_tris`.
///
/// Note that there is no failure state,
/// this is guaranteed to fill in every triangle.
///
/*
fn polyfill_calc(coords: &Vec<[Real; 2]>, coords_sign: ESign, r_tris: &mut Vec<[usize; 3]>) {
    let indices: Vec<PolyIndex> = Vec::with_capacity(coords.len());

    let mut pf = prepare(coords, coords_sign, r_tris, indices);

    calc(&mut pf);
}
*/

pub fn triangle_indices(coords: &Vec<[Real; 2]>, coords_sign: ESign) -> Vec<[usize; 3]> {
    let mut r_tris = Vec::with_capacity(poly_tri_count!(coords.len()));

    let indices: Vec<PolyIndex> = Vec::with_capacity(coords.len());

    {
        let mut pf = prepare(coords, coords_sign, &mut r_tris, indices);
        calc(&mut pf);
    }
...
//     polyfill_calc(&coords, 0, &mut tris);

//     /*
//     match write_svg("test", &coords, &tris) {
//         Ok(_) => (),
